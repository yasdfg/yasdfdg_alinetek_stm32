...
U8 W25QXX_spiReadByte(void){
	return 0;
}
void W25QXX_spiWriteByte(U8 data){
...
void W25QXX_writeSR(U8 sr){
	SPI_FLASH_CS=0;                            //使能器件
	W25QXX_spiReadByte(W25X_WriteStatusReg);   //发送写取状态寄存器命令
	W25QXX_spiReadByte(sr);                    //写入一个字节
	SPI_FLASH_CS=1;                            //取消片选
}
//SPI_FLASH写使能
//将WEL置位
...
{
	SPI_FLASH_CS=0;                            //使能器件
	W25QXX_spiReadByte(W25X_WriteEnable);      //发送写使能
	SPI_FLASH_CS=1;                            //取消片选
}
//SPI_FLASH写禁止
//将WEL清零
